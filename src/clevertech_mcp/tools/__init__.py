...
Each tool category receives the shared ``CleverTechClient``, ``config``
dict, and ``LocalRateLimiter`` via closure.
"""
...
def register_all_tools(
    ...
    try:
        ...
        register_property_tools(mcp, client, config, rate_limiter)
    except ImportError:
        pass

    try:
        from clevertech_mcp.tools.dls import register_dls_tools
        register_dls_tools(mcp, client, config, rate_limiter)
    except ImportError:
        pass

    try:
        from clevertech_mcp.tools.building import register_building_tools
        register_building_tools(mcp, client, config, rate_limiter)
    except ImportError:
        pass

    try:
        from clevertech_mcp.tools.zoning import register_zoning_tools
        register_zoning_tools(mcp, client, config, rate_limiter)
    except ImportError:
        pass

    try:
        from clevertech_mcp.tools.business import register_business_tools
        register_business_tools(mcp, client, config, rate_limiter)
    except ImportError:
        pass

    try:
        from clevertech_mcp.tools.geo import register_geo_tools
        register_geo_tools(mcp, client, config, rate_limiter)
    except ImportError:
        pass

    try:
        from clevertech_mcp.tools.meta import register_meta_tools
        register_meta_tools(mcp, client, config, rate_limiter)
    except ImportError:
        pass
